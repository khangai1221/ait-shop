...
function walk(node: ts.Node, depth: number) {
  if (ts.isJsxElement(node)) {
    ...
    if (!close) {
      console.log(
        `Unclosed JSX element: ${openName} at line ${open.getLine(sourceFile)}, col ${open.getCol(sourceFile)}`,
      );
    } else {
      const closeName = close.tagName.getText(sourceFile);
      if (closeName !== openName) {
        console.log(
          `Mismatched JSX: <${openName}> closed by </${closeName}> at line ${close.getLine(sourceFile)}`,
        );
      }
    }
  }
  ...
  if (
    ...
    node.text === "collapsed" &&
    node.getLine(sourceFile) >= 320 &&
    node.getLine(sourceFile) <= 345
  ) {
    console.log(
      `Identifier 'collapsed' at line ${node.getLine(sourceFile)}, col ${node.getCol(sourceFile)}`,
    );
  }
